...
Okay, I need to do all the hyperparameter tuning stuff and run the model again. I can subset by Grand Slams too.
...
test_data_2$predicted_winner <- predictions_rf3

player1_wins <- test_data_2[test_data_2$Match_Winner == 1, ]
win_counts <- table(player1_wins$Player1)
sorted_counts <- sort(win_counts, decreasing = TRUE)
sorted_counts
```

This shows Nadal, Djokovic and Federer now. This is just the number of wins for the players so a better metric would be to get the win percentage. I think I can do this by getting the number of matches each of the (maybe top 10) players played in the test data and then go wins divided by matches. 
