...
top_50_again <- model_data %>% filter(winner_id %in% temp2$winner_id)
top_50_again$surface_number <- as.factor(top_50_again$surface_number)
top_50_again$tournament_id <- as.factor(top_50_again$tournament_id)
top_50_again$Player1 <- as.factor(top_50_again$Player1)
top_50_again$Player2 <- as.factor(top_50_again$Player2)
top_50_again$Match_Winner <- as.factor(top_50_again$Match_Winner)
```
...
top_50_3 <- model_data %>% filter(winner_name %in% temp3$winner_name)
top_50_3 <- as.factor(top_50_3)

top_50_3$surface_number <- as.factor(top_50_3$surface_number)
top_50_3$tournament_id <- as.factor(top_50_3$tournament_id)
top_50_3$Player1 <- as.factor(top_50_3$Player1)
top_50_3$Player2 <- as.factor(top_50_3$Player2)
top_50_3$Match_Winner <- as.factor(top_50_3$Match_Winner)
...
```{r default again}
set.seed(123)  # Set seed for reproducibility
train_indices_2 <- sample(1:nrow(model), 0.7 * nrow(model)) #training set 70% and testing set 30%
train_data_2 <- model[train_indices_2, ]
test_data_2 <- model[-train_indices_2, ]
...
test_data_2$predicted_winner <- predictions_rf2
player_wins_2 <- aggregate(predicted_winner ~ Player1, data = test_data_2, FUN = sum)
```
...
vip::vip(xgb.fit.default)


xgb_recipe <- recipe(Match_Winner ~ ., data = train_data)
# Prepare the test data using the same preprocessing steps as the training data
test_prep <- prep(xgb_recipe, newdata = test_data, retain = TRUE) %>%
  step_integer(all_nominal()) %>%
  juice()

# Create a matrix representation for the test features
X_test <- as.matrix(test_prep[, setdiff(names(test_prep), "Match_Winner")])

# Extract the test target variable
Y_test <- test_prep$Match_Winner
...
min(tennis_xgb1$evaluation_log$test_rmse_mean)
```
